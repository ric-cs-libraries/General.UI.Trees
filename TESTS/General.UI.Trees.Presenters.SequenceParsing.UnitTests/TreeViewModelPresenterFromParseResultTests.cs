...
public class TreeViewModelPresenterFromParseResultTests
{
    ...
    public void GetViewModel__ShouldReturnTheCorrectlyFilledTreeViewModel()
    {
        ...
        IParseResultTreeElementToTreeViewModelElementDataConverter<Fixtures.TreeViewModelElementsData> toDataConverter =
            new Fixtures.ParseResultTreeElementToTreeViewModelElementDataConverter();

        var treeTitle = "Essai1";
        TreeViewModelPresenterFromParseResult<Fixtures.TreeViewModelElementsData> presenter = new(toDataConverter, treeTitle);

        General.Basics.Trees.GenericTree.TreeElement<Fixtures.TreeViewModelElementsData> element;
        General.Basics.Trees.GenericTree.Node<Fixtures.TreeViewModelElementsData> rootParentElement, parentElement, parentElement2;

        //--- Act ---
        TreeViewModel<Fixtures.TreeViewModelElementsData> result = presenter.GetViewModel(parseResult);

        //--- Assert ---
        rootParentElement = result.Tree;
        Assert.IsType<General.Basics.Trees.GenericTree.GenericTree<Fixtures.TreeViewModelElementsData>>(rootParentElement);
        ...
        Assert.Equal($"nbChildren=5; id='0'; descr='{treeTitle}'", rootParentElement.Data!.Hint);
        {
            ...
            Assert.IsType<General.Basics.Trees.GenericTree.Leaf<Fixtures.TreeViewModelElementsData>>(element);
            ...
            Assert.IsType<General.Basics.Trees.GenericTree.Node<Fixtures.TreeViewModelElementsData>>(element);
            Assert.Equal(0, element.Data!.Depth);
            ...
            Assert.Equal($"nbChildren=1; id='2'; parentId='0'; indexInParent=1; descr=' StartDelim(2)=`AA`; EndDelim(3)=`AAF`'", element.Data!.Hint);
            {
                ...
                parentElement = (element as General.Basics.Trees.GenericTree.Node<Fixtures.TreeViewModelElementsData>)!;
            ...
            Assert.Equal($"nbChildren=3; id='5'; parentId='0'; indexInParent=3; descr=' StartDelim(2)=`BB`; EndDelim(3)=`BBF`'", element.Data!.Hint);
            {
                ...
                {
                    parentElement2 = (element as General.Basics.Trees.GenericTree.Node<Fixtures.TreeViewModelElementsData>)!;
